...
from tools.reversible_delete_broker import ReversibleDeleteBrokerEndpoint
...
def install_reversible_delete_runtime(
    *,
    ...
    inherited_path: str,
    broker_endpoint: ReversibleDeleteBrokerEndpoint,
    ...
    for executable in ("rm", "unlink", "rmdir"):
        ...
        args = [
            *common,
            ...
            "--broker-host",
            shlex.quote(broker_endpoint.host),
            "--broker-port",
            shlex.quote(str(broker_endpoint.port)),
            "--broker-token",
            shlex.quote(broker_endpoint.token),
            "--",
            '"$@"',
        ]
        script = (
