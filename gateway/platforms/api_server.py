...
def _tool_result_preview(result: Any, limit: int = TOOL_PROGRESS_OUTPUT_PREVIEW_CHARS) -> str:
    """Return a bounded text preview for SSE tool-completion events."""
    if result is None:
        return ""
    ...
    async def _write_sse_chat_completion(
        ...
        try:
            ...
            try:
                ...
                if not sent_content:
                    last_activity = await _emit(f"Error running Hermes agent: {exc}")

            final_response = ""
            if isinstance(result, dict):
                final_response = str(result.get("final_response") or result.get("error") or "")
            if final_response and not sent_content:
